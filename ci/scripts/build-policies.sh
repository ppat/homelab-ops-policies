...
#
# Why this exists (design doc phase2.0-policy-migration-design-v3.md §6.3): what a consumer of
# this repo actually applies is the *built* output of a profile directory -- pure policy files
# plus, once they exist, the exemptions/ kustomize patches -- never a raw policy file in
# isolation and never a raw patch file on its own (a patch alone isn't valid Kyverno YAML). Both
# test tiers (`kyverno test`/`kyverno apply` in structural-smoke-check, Chainsaw's `apply: file:`)
# need to exercise that same built output, so this script is the one place that logic lives
# rather than being duplicated inline in two CI jobs and by hand for local runs.
#
# Output files are named after each resource's `metadata.name`, not its source file's basename
# -- kustomize build only hands back the built stream, not which source file a given resource
# came from, and (verified against the current, pre-migration policy content) the two don't
# always match today (e.g. best-practices/restrict-node-port.yaml currently carries
# `metadata.name: restrict-nodeport`). `metadata.name` is the one identifier that survives the
# build and is guaranteed stable and unique within a profile (kustomize itself would refuse to
# build a profile with two resources sharing a name+kind), so it's what test files should
# reference. House style set by Wave C of the migration is expected to make `metadata.name`
# equal to the file basename going forward, at which point this doubles as filename-preserving
# too -- but this script does not assume that; it only ever trusts `metadata.name`.
set -euo pipefail

...
# per-profile ready-smoke Chainsaw tests (design doc phase2.0-policy-migration-design-v3.md §6.4,
# "Profile ready-smoke"): the cheapest cluster-level catch for engine-level rejects -- CEL the
# webhook refuses, RBAC the background controller lacks -- across every policy at once.
#
...
#   2. TWO DIFFERENT STATUS SHAPES, and the set of files is different in every wave of the
#      migration. Legacy `ClusterPolicy`/`ClusterCleanupPolicy` report readiness as a
#      `Ready` entry in `.status.conditions[]`; the new `policies.kyverno.io` kinds report it as
#      the boolean `.status.conditionStatus.ready` (with `WebhookConfigured` and
#      `RBACPermissionsGranted` entries under `.status.conditionStatus.conditions[]`). Both are
#      handled below, so the same test file keeps working as policies migrate one at a time and
#      needs no edit per wave.
#
...
# Measured on a stock chart 3.8.2 install: a `ClusterCleanupPolicy` sits at `status: null`
